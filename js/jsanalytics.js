...
//  var OAUTH2_CLIENT_ID = '253617064312-g44bqh9e0ic3e987qsmcctu32apa75nh.apps.googleusercontent.com';
//  var OAUTH2_SCOPES = [
      ...
      if ('error' in response) {
        ...
        $.each(response.items, function() {
          ...
          aElement.click(function() {
            getUserActivity(videoId, title);
            displayDemographics(videoId, title);
            displayGeographicViews(videoId, title);
          });
...
function getUserActivity(videoId, title) {
    if (channelId) {
      ...
      request.execute(function(response) {
            ...
            if ('error' in response) {
                ...
                var views = response.rows["0"][1];
                var likes = response.rows["0"][2];
                var dislikes = response.rows["0"][3];
...
// Requests for Gender Group views
function displayDemographics(videoId, title) {
    if (channelId) {
        // To use a different date range, modify the ONE_MONTH_IN_MILLISECONDS
        // variable to a different millisecond delta as desired.
        var today = new Date();
      ...
      request.execute(function(response) {
            ...
            if ('error' in response) {
                ...
                console.log(response);
                displayChartGender(videoId, response, title);
            }
        });
    } 
    else {
...
// Requests YouTube Analytics for a video, and displays results in a chart.
// Requests for Geographic views
function displayGeographicViews(videoId, title) {
    if (channelId) {
        // To use a different date range, modify the ONE_MONTH_IN_MILLISECONDS
        // variable to a different millisecond delta as desired.
        var today = new Date();
        ...
        request.execute(function(response) {
            ...
            if ('error' in response) {
                ...
                displayChartGeographic(videoId, response, title);
            }
        });
    }
    else {
...
function displayChartGeographic(videoId, response, title) {
    if ('rows' in response) {
    hideMessage();
    ...
    var chart = new google.visualization.GeoChart(document.getElementById('geoGraphicView'));
...
function displayChartGender(videoId, response, title) {
    if ('rows' in response) {
        hideMessage();
        ...
        var f24 = response.rows["0"][2];
        var m24 = response.rows[1][2];
        var m34 = response.rows[2][2];
        ...
        var chart = new google.visualization.BarChart(document.getElementById('demographics'));
        chart.draw(chartDataTable, options);
        //Additional options can be set if desired.
        //See https://developers.google.com/chart/interactive/docs/reference#visdraw
        //width: 800, height: 300, is3D: false,
        //title: 'Gender group view %: ' + title
...
function displayMessage() {
    $('#message').text('Sign in required, sign in with google above then refresh the page').show();
}
